<script>
  ...
  import { compactLabel, formatNumber, formatPercent, labelName } from '../lib/presentation.js';
  ...
  function detailText(row) {
    ...
    if (detailKind === 'coverage') {
      return `Coverage in the analytical table is ${formatPercent(row.non_null_share)} and missing share is ${formatPercent(row.missing_share)}.`;
    }

    if (detailKind === 'completeness') {
      return `Average completeness across ${formatNumber(row.countries, 0)} countries and ${formatNumber(row.rows, 0)} country-year rows.`;
    }

    if (detailKind === 'residual') {
      return `Mean absolute prediction error is ${formatNumber(row.mean_abs_error, 2)} years across ${formatNumber(row.rows, 0)} rows.`;
    }
...
<section class="chart-card">
  ...
  <div class="chart-body">
    ...
    <aside class="detail">
      ...
      {#if detailKind === 'coverage'}
        <p class="meta"><GlossaryTerm term="coverage" label="Coverage" /> reflects how often a variable is available.</p>
      {:else if detailKind === 'completeness'}
        <p class="meta"><GlossaryTerm term="completeness" label="Completeness" /> helps explain where comparisons are better supported.</p>
      {:else if detailKind === 'residual'}
        <p class="meta"><GlossaryTerm term="mae" label="Mean absolute error" /> highlights where the model needs more local context.</p>
      {/if}
    </aside>
  </div>
</section>
